/**
 * batchSlice.ts — Bulk subscription operations slice (slices pattern).
 *
 * Migrated from app/stores/batchStore.ts (Issue #944). Supports batch create
 * from CSV/JSON, batch update with filtering, batch cancel with reason
 * collection, batch charge for manual billing, per-item status tracking,
 * atomic execution, post-commit rollback, idempotent retry, per-operation-type
 * configuration, success/timing analytics, result export, and audit history.
 *
 * The clear-history action is exposed as `clearBatchHistory` in the combined
 * store to avoid a name clash with the transaction slice's `clearHistory`;
 * the legacy `useBatchStore` adapter re-exposes it under the original name.
 */
...
export interface BatchSlice {
  ...
  clearBatchHistory: () => Promise<void>;
...
export type BatchStoreState = AppState;
...
export const createBatchSlice: SliceCreator<BatchSlice> = (set, get) => ({
  draft: emptyDraft(),
  currentResult: null,
  history: [],
  ...
  loadHistory: async () => {
    // Automatically handled by the root persist middleware.
  },
  ...
  clearBatchHistory: async () => {
    set({ history: [] });
    await clearPersistedBatchHistory();
  },
