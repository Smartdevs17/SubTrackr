/**
 * paymentSlice.ts — Payment method + fallback chain slice (slices pattern).
 *
 * Migrated from app/stores/paymentStore.ts (Issue #944). Manages payment
 * methods with priority ordering, explicit fallback chains, expiry alerts and
 * payment analytics. The analytics action is exposed as `getPaymentAnalytics`
 * in the combined store to avoid a name clash with the usage-analytics action
 * on the metering slice; the legacy `usePaymentStore` adapter re-exposes it
 * under the original `getAnalytics` name.
 */
...
export interface PaymentSlice {
  ...
  // Analytics (renamed from `getAnalytics` — see header comment)
  getPaymentAnalytics: () => PaymentAnalytics;
}

export type PaymentStoreState = AppState;

export const createPaymentSlice: SliceCreator<PaymentSlice> = (set, get) => ({
  methods: [],
  attemptLog: [],
  chains: [],
  ...
  setPriority: (id, priority) => {
    set((s) => ({
      methods: s.methods.map((m) => (m.id === id ? { ...m, priority, updatedAt: Date.now() } : m)),
    }));
  },

  setExpiry: (id, expiresAt) => {
    set((s) => ({
      methods: s.methods.map((m) => (m.id === id ? { ...m, expiresAt, updatedAt: Date.now() } : m)),
    }));
  },
  ...
  updateChain: (id, updates) => {
    set((s) => ({
      chains: s.chains.map((c) => (c.id === id ? { ...c, ...updates, updatedAt: Date.now() } : c)),
    }));
  },
  ...
  reorderChain: (id, methodIds) => {
    set((s) => ({
      chains: s.chains.map((c) => (c.id === id ? { ...c, methodIds, updatedAt: Date.now() } : c)),
    }));
  },
  ...
  chargeWithChain: (chainId, amount) => {
    ...
    const byId = new Map(get().methods.map((m) => [m.id, m]));
    const ordered =
      chain.maxAttempts > 0 ? chain.methodIds.slice(0, chain.maxAttempts) : chain.methodIds;
