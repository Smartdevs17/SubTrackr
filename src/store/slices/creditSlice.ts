/**
 * creditSlice.ts — Subscriber account credit slice (slices pattern).
 *
 * Migrated from app/stores/creditStore.ts (Issue #944) so credit state and
 * actions live in the combined useAppStore. Mirrors the `subtrackr-credit`
 * Soroban contract: credit is held in lots that can expire, applied to charges
 * oldest-first, transferred between accounts, and is fully auditable.
...
export type CreditStoreState = AppState;
...
export const createCreditSlice: SliceCreator<CreditSlice> = (set, get) => {
