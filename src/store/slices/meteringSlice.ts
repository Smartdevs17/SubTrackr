/**
 * meteringSlice.ts — Usage tracking and usage-based billing slice.
 *
 * Migrated from app/stores/meteringStore.ts (Issue #944). Mirrors the
 * `subtrackr-metering` Soroban contract: reporters push usage increments per
 * (subscription, metric); the slice aggregates into period buckets, fires
 * alerts on thresholds, exposes trends, and computes usage charges.
 */
...
export const billableUnits = (used: number, included: number): number =>
...
export interface MeteringSlice {
  ...
  getUsageHistory: (
    subscriptionId: string,
    metric?: string
  ) => { subscriptionId: string; metric: string; value: number; timestamp: number }[];
...
export type MeteringStoreState = AppState;
...
export const createMeteringSlice: SliceCreator<MeteringSlice> = (set, get) => {
  ...
  return {
    ...
    getUsageTrends: (sub) => {
      ...
      const previousHistory = get().usageHistory.filter(
        (h) =>
          h.subscriptionId === sub && h.timestamp >= sixtyDaysAgo && h.timestamp < thirtyDaysAgo
      );
