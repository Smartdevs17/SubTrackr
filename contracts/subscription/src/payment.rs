...
pub fn charge_subscription(env: &Env, storage: &Address, subscription_id: u64) {
    ...
    if subscription_lifecycle::check_and_resume(env, &mut sub) {
        storage_persistent_set(
            env, storage, StorageKey::Subscription(subscription_id), sub.clone(),
        );
    }

    assert!(sub.status == SubscriptionStatus::Active, "Subscription not active");
    ...
    revenue::generate_revenue_schedule(
        env, storage, subscription_id, sub.plan_id, charge_price, now, plan.interval.seconds(),
    );
    ...
    if let Some(invoice_addr) = get_invoice_contract(env, storage) {
        let period = TimeRange { start: sub.last_charged_at, end: sub.next_charge_at };
        let _invoice: Invoice = env.invoke_contract(
            &invoice_addr,
            &Symbol::new(env, "generate_invoice"),
            soroban_sdk::vec![
                env, storage.clone().into_val(env), subscription_id.into_val(env),
                period.into_val(env), String::from_str(env, "GLOBAL").into_val(env),
...
pub fn request_refund(env: &Env, storage: &Address, subscription_id: u64, amount: i128) {
    let mut sub: Subscription =
        storage_persistent_get(env, storage, StorageKey::Subscription(subscription_id))
            .expect("Subscription not found");
    ...
    assert!(amount <= sub.total_paid, "Refund amount cannot exceed total paid");
